...
@login_required
def give_review(request, request_id):
    service_request = get_object_or_404(ServiceRequest, id=request_id, requester=request.user, status='Completed')
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            review.reviewed_user = service_request.offer.user
            review.save()
    ...
    return render(request, 'give_review.html', {'form': form, 'service': service_request.offer})

@login_required
def complete_request(request, request_id):
    req = get_object_or_404(ServiceRequest, id=request_id)

    if req.status != "Accepted" or req.offer.user != request.user:
        messages.error(request, "You can't complete this request.")
        return redirect('dashboard')

    requester_profile = req.requester.userprofile
    provider_profile = req.offer.user.userprofile

    if requester_profile.time_balance < req.requested_hours:
        messages.error(request, "Requester doesn't have enough hours.")
        return redirect('dashboard')

    # Update balance
    requester_profile.time_balance -= req.requested_hours
    provider_profile.time_balance += req.requested_hours
    requester_profile.save()
    provider_profile.save()

    # Record transaction
    TimeTransaction.objects.create(user=req.requester, hours=-req.requested_hours,
                                   description=f"Requested service: {req.offer.title}")
    TimeTransaction.objects.create(user=req.offer.user, hours=+req.requested_hours,
                                   description=f"Provided service to {req.requester.username}")

    # Handle meeting link, file, date & time
    req.meeting_link = request.POST.get("meeting_link")
    req.service_date = request.POST.get("service_date")
    req.service_time = request.POST.get("service_time")

    if 'delivered_file' in request.FILES:
        req.delivered_file = request.FILES['delivered_file']

    req.status = "Completed"
    req.save()

    messages.success(request, "Service marked as completed and shared.")
    return redirect('dashboard')
